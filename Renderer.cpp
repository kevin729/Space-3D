...
void render() {
	...
	for (auto it = entities.begin(); it != entities.end(); ++it) {
		...
		glVertexAttribPointer(shader.vertexLocation, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 3, 0);
		
		glBindBuffer(GL_ARRAY_BUFFER, model.textureCoBuffer);
		glVertexAttribPointer(shader.textureCoLocation, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 2, 0);

		glBindBuffer(GL_ARRAY_BUFFER, model.normalBuffer);
		glVertexAttribPointer(shader.normalLocation, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 3, 0);
